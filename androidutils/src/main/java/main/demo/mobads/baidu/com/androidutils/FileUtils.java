...
public class FileUtils {
    ...
    public static boolean isSdcardNormal() {
        ...
        File file = new File(SDCARD_PATH + "test.txt");
    ...
    public static boolean copyFileByInputStream(InputStream in, File dst) {
        if (in == null || dst == null || dst.isDirectory()) {
            return false;
        } else if (dst.exists() && dst.length() > 0) {
            return true;
        } else {
            FileOutputStream out = null;
            try {
     ...
     * @param object
     */
    public static void writeObject(String path, String name, Object object) {
        try {
            File file = createFile(path, name);
